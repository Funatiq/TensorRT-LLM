    ...
    bool computeLogProbs)
{
    ...
    newRequests(
        inputBuffers.setupBatchSlots, requests, samplingConfigs, modelConfig, decoder, *streamPtr, maxSeqLength, decoderState);
    ...
    checkSequenceLengths(*decoderState.getSequenceLengths(), expectedLengths, manager);
    EXPECT_THAT(
        getFinished(*decoderState.getFinishedSum(), samplingConfigs, manager), ::testing::Each(true));
    ...
    std::vector<SizeType32> const& acceptedTokensPerStep, SizeType32 maxGeneratedTokensPerStep)
{
    ...
    checkSequenceLengths(*decoderState.getSequenceLengths(), expectedLengths, manager);
    EXPECT_THAT(
        getFinished(*decoderState.getFinishedSum(), samplingConfigs, manager), ::testing::Each(false));
